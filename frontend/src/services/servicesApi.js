...
function normalizeSeverity(value) {
  ...
  if (normalized.includes("crít") || normalized.includes("crit"))
    return { status: "critical", label: "Crítico" };
  if (normalized.includes("mayor") || normalized.includes("major"))
    return { status: "warning", label: "Mayor" };
  if (normalized.includes("menor") || normalized.includes("minor"))
    return { status: "warning", label: "Menor" };
...
function getServiceIcon(name) {
  ...
  if (normalized.includes("bff") || normalized.includes("gateway"))
    return "gateway";
  return "services";
}
...
function normalizeService(service, index) {
  ...
  const statusMeta = normalizeStatus(
    getFirstDefined(service.status, service.estado, service.health),
  );
  const rawTrend = toArray(
    getFirstDefined(
      service.trend,
      service.tendencia,
      service.history,
  ...
  return {
    ...
    uptime,
    uptimeLabel:
      uptime !== null ? `${percentFormatter.format(uptime)}%` : "Sin dato",
    ...
    trend: rawTrend.map(
      (item) => Number(getFirstDefined(item.value, item.valor, item)) || 0,
    ),
  };
}

function normalizeIncident(incident, index) {
  const severityMeta = normalizeSeverity(
    getFirstDefined(incident.severity, incident.severidad, incident.tipo),
  );
  const statusMeta = normalizeStatus(
    getFirstDefined(incident.status, incident.estado),
  );
  ...
  return {
    ...
    status: statusMeta.status,
    statusLabel: statusMeta.label,
    time: getFirstDefined(
      incident.time,
      incident.fecha,
      incident.createdAt,
      incident.fechaDeteccion,
    ),
    timeLabel: formatDate(
      getFirstDefined(
        incident.time,
        incident.fecha,
        incident.createdAt,
...
function normalizeEvent(event, index) {
  const statusMeta = normalizeStatus(
    getFirstDefined(event.status, event.estado, event.tipo),
  );
  ...
  return {
    ...
    statusLabel: statusMeta.label,
    timeLabel: formatDate(
      getFirstDefined(event.time, event.fecha, event.createdAt),
    ),
  };
}
...
export function normalizeServicesResponse(payload) {
  ...
  return {
    ...
    incidents: toArray(
      getFirstDefined(payload?.incidents, payload?.incidentes),
    ).map(normalizeIncident),
    events: toArray(getFirstDefined(payload?.events, payload?.eventos)).map(
      normalizeEvent,
    ),
    availabilityHistory: toArray(
      getFirstDefined(
...
export async function getServicesStatus() {
  const response = await requestWithTimeout(
    `${API_BASE_URL}/api/dashboard/services`,
  );
