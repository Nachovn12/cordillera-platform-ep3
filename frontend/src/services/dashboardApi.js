...
export function normalizeBffStatus(value) {
  ...
  if (normalized === 'OK' || normalized === 'OPERATIVO' || normalized === 'UP') {
  ...
  return { raw: value, status: 'info', label: value ? String(value) : 'Estado no informado' }
}

function normalizeKpi(kpi, index) {
  const rawValue = getFirstDefined(kpi.valor, kpi.value, kpi.valorActual, kpi.currentValue)
  const unit = getFirstDefined(kpi.unidad, kpi.unit, '')
  const statusLabel = getFirstDefined(kpi.estado, kpi.status, kpi.statusLabel, 'Estado no informado')
  ...
  return {
    ...
    category,
    value: unit === 'CLP' ? formatClp(rawValue).replace(/\s/g, '') : String(rawValue ?? 'Sin datos disponibles'),
    unit,
    status,
    statusLabel,
...
function normalizeAlert(alert, index) {
  ...
  return {
    ...
    description: getFirstDefined(alert.mensaje, alert.message, alert.descripcion, alert.description, ''),
    category: getFirstDefined(alert.categoria, alert.category, type),
    severity: normalizedType.includes('critical') || normalizedType.includes('crítica')
      ? 'critical'
      : normalizedType.includes('warning') || normalizedType.includes('advert')
        ? 'warning'
        : 'info',
...
function normalizeService(service, index) {
  ...
  const rawStatus = getFirstDefined(service.status, service.estado, service.statusLabel)
  ...
  return {
    ...
    shortDetail: getFirstDefined(service.detalle, service.detail, service.descripcion, 'Estado informado por BFF'),
  }
}
