...
function getSummaryMetrics(kpis) {
  ...
  return [
    ...
    },
    {
      ...
      value: averageVariation === null ? 'Sin datos' : `${averageVariation > 0 ? '+' : ''}${averageVariation.toLocaleString('es-CL', { maximumFractionDigits: 1 })}%`,
...
function getEvolutionData(kpis) {
  ...
  return {
    ...
    series: withHistory.slice(0, 3).map((kpi, index) => ({
      ...
      values: kpi.history.map((item) => Number(item.valor ?? item.value ?? item.cumplimiento ?? 0)),
    })),
  }
}
...
export default function KpisScreen({ onBffStatusChange }) {
  ...
  return (
    <main className="screen screen--kpis">
      ...
      <section className="content-grid content-grid--kpis" aria-label="Evolución y estado de integración">
        <TrendPanel
          type="line"
      ...
      <section className="content-grid content-grid--table" aria-label="Detalle y filtros">
        <div className="panel panel--table">
          <SectionHeader title="Detalle de indicadores" description="Resumen de los indicadores estratégicos entregados por el BFF Gateway." />
                ...
                {kpis.map((kpi) => {
                  ...
                  return (
                    <tr key={kpi.id}>
                      ...
                      <td className={isNegative ? 'table-variation--negative' : 'table-variation--positive'}>{kpi.change}</td>
                      <td>
